...
def process_predictions(
    ...
    for hypo in hypos[: min(len(hypos), args.nbest)]:
        ...
        utt_wer = editdistance.eval(hyp_words, tgt_words) * 100.0 / len(tgt_words)
        
        if res_files is not None:
            print(
                "{} ({}-{}-{})".format(utt_wer, editdistance.eval(hyp_words, tgt_words), len(tgt_words), id), file=res_files["utt.wer"]
            )
...
def prepare_result_files(args):
    ...
    return {
        ...
        "utt.wer": get_res_file("utt.wer"),
            "wer": get_res_file("wer")
    }
...
#             arg_overrides=overrides,
    ...
    if args.dump_emissions:
        ...
        if res_files is not None:
            print(
                "{}-{}-{}-{}".format(wer, lengths_t, num_sentences, args.gen_subset),
                file=res_files["wer"],
            )
